    ...
    if request.method == 'POST':
        ...
        if copy:
            ...
            Borrow.objects.create(
                ...
                due_date=timezone.now() + timezone.timedelta(days=28)
            )
...
def pay_overdue_fees(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            overdue_fee = form.save(commit=False)
            overdue_fee.paid = True  # Mark the fee as paid
            overdue_fee.save()  # Save the changes to the database
            # Redirect to a confirmation page or back to the dashboard with a success message
            return redirect('client_dashboard', {'message': 'Overdue fee paid successfully.'})
    else:
        form = OverdueFeeForm()
